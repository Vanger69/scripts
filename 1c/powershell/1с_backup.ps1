# Бэкап баз данных 1с на яндекс.диск. Папка для сохранения выгрузок должна быть установлена как "доступная локально".

# Список баз данных
$Bases = @(
    @{
        Path     = "d:\1С Базы\Base1\"   # Путь к первой базе
    },
    @{
        Path     = "d:\1С Базы\Base2\"   # Путь ко второй базе
    },
    @{
        Path     = "d:\1С Базы\Base3\"   # Путь к третьей базе
    }
)
...
$BackupFolder = "C:\Users\User\YandexDisk-***\backup"

# Максимальное количество резервных копий
$MaxBackups = 9

# Задержка перед удалением старых файлов (в секундах)
$DelayBeforeDeletion = 5
...
function RotateBackups {
    ...
    # Получить список файлов в папке резервных копий, отсортированный по времени изменения (старые файлы первыми)
    $BackupFiles = Get-ChildItem -Path $BackupFolder -File | Sort-Object -Descending -Property LastWriteTime
    $FilesToRemove = @()

    # Проверить, если количество файлов превышает лимит
    if ($BackupFiles.Count -gt $MaxBackups) {
        $FilesToRemove = $BackupFiles | Select-Object -Last ($BackupFiles.Count - $MaxBackups)
        
		# Задержка перед удалением
        Write-Output "Ожидание $DelayBeforeDeletion секунд перед удалением старых файлов..."
        Start-Sleep -Seconds $DelayBeforeDeletion
		
        # Удалить старые файлы
        foreach ($File in $FilesToRemove) {
            Write-Output "Удаление старого резервного файла: $($File.FullName)"
...
foreach ($Base in $Bases) {
    ...
    $BackupPath = Join-Path -Path $BackupFolder -ChildPath ("{0}_{1}.dt" -f (Split-Path -Leaf $BasePath), (Get-Date -Format 'yyyyMMdd_HHmmss'))

    # Формирование команды для выгрузки
    $Command = "`"$OneCExecutable`" DESIGNER /F `"$BasePath`" /DumpIB `"$BackupPath`""
...
# Выполнить ротацию резервных копий
RotateBackups -BackupFolder $BackupFolder -MaxBackups $MaxBackups
